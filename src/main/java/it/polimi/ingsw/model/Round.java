    ...
    public void addStudentOnTable(int playerId, int studentIndex) {
        try {
            ...
            if (game instanceof ExpertGame){
                if (game.getGameTable().getSchoolBoards()[playerId].getNumberOfStudentsOnTable(color) % 3 == 0 && game.getGameTable().getSchoolBoards()[playerId].getNumberOfStudentsOnTable(color) % 3 != 1) ((ExpertGame)game).addCoinToAPlayer(playerId);
            }
